...
> **Sep 2026 (v5):** **Datadog** monitors, a first-class SLO object and a dashboard as code — the third backend for the same golden signals, after CloudWatch and Splunk. A test asserts the Datadog SLO target matches the number the CloudWatch burn-rate alarms compute against, because two backends disagreeing about the target is worse than having one. Off by default; 8 terraform tests.
>
