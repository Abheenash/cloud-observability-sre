...
- **Synthetics canary** (`sfs-uptime`, [`terraform/canary.tf`](terraform/canary.tf)) — a CloudWatch Synthetics canary that loads the live app and asserts a 2xx, providing outside-in uptime that's independent of the service's own metrics. It is the one component here with a real recurring cost, so it is run **build → prove → destroy** (see [Trade-offs](#trade-offs)): provisioned as Terraform and stood up on demand for a demo, then torn down so the layer sits at $0 at rest.
...
- **Synthetic-check cost and cadence.** The canary is real recurring cost, unlike the mostly-free-tier metrics/logs/dashboards. I originally ran it every 5 minutes for tight outside-in coverage, then changed it to **hourly** — documented in `canary.tf` as roughly **$0.86/mo hourly vs. ~$10/mo at 5-minute**. For a hobby-scale app that's the right trade: I accept coarser outside-in resolution to keep the observability layer near-zero cost, backed by a budget alarm on the account. Taken to its conclusion, the canary now follows a **build → prove → destroy** loop — the same cost-controlled model as [aws-eks-platform](https://github.com/Abheenash/aws-eks-platform): it is provisioned as Terraform and stood up on demand for a demo (`terraform apply`), then torn down so the observability layer sits at $0 at rest. It is currently torn down; every other signal — dashboard, alarms, X-Ray, RUM — stays live.
